...
library(corrplot)
library(dplyr)
data = data %>% filter(gestational_age <= 46)
X=data %>% dplyr::select(-albumin,-gestational_age)
y=data %>% dplyr::select(gestational_age)
predictors=na.omit(X[,1:12])
X=na.omit(X)
```
Correlation heat map. There are relatuvely high dependency between pcb variables, which may cause the multicolinearity issue. The association between treatment variables and tri or cho is weak.categorical_data_analysis???edition_3.pdf
```{r}
mea=X %>% dplyr::select(dde:pcb_203,triglycerides,cholesterol)
corrplot(cor(mea))
```
Boxplot by different centers.Heterogenity exists across centers. May add a random effect for different centers.
```{r}
...
boxplot(gestational_age~center,data=data)
```
Evalute the marginal association. Very clear marginal association between dde and preterm. 
```{r}
library(ggplot2)
data$preterm=data$gestational_age<37
ggplot(data, aes(x=preterm, y=dde)) +  geom_boxplot()
