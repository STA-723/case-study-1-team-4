...
data$preterm = cut(data$gestational_age, 
                   c(-Inf, dangerous_preterm-1, preterm-1, Inf), c(2, 1, 0))
data %>%
  ggplot(aes(x=center, fill = as.factor(preterm))) +
  geom_bar(position = "fill")
@
<<>>=
...
lnewdat <- melt(data, id.vars = c("dde_lipid"),variable.name = "Level", value.name="Probability")
## view first few rows
head(lnewdat)
@
