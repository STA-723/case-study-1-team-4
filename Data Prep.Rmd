...
d_subset= d %>% 
  dplyr::select(gestgroup, dde_env_bc,
                pcb_env_bc,race,maternal_age,score_occupation,
                center,score_income,score_education, smoking_status)
cumlogit_model_env_full = polr(gestgroup ~ . + center*dde_env_bc 
                               + center*pcb_env_bc +
                                 race*dde_env_bc +
                                 race*pcb_env_bc, data=d_subset,
                          method = c("logistic"))
...
cumlogit_model_env=polr(
  gestgroup ~ dde_env_bc + pcb_env_bc + race + maternal_age + score_occupation + center + score_income + score_education, 
  d, 
  method = c("logistic"))
...
predict_DDE_w = tibble(dde = seq(0,k-1),
  ...
  gather(Outcome, Probability,p0,p1,p2) %>%
    mutate(Outcome = case_when(Outcome == "p0" ~ "Dangerous",
                             Outcome == "p1" ~ "Pre term", 
                             TRUE~"At term") %>% fct_relevel("Dangerous", "Pre term", "At term"))%>%
...
```
```{r}
#ggarrange(predict_DDE, predict_PCB, ncol=2,common.legend = TRUE , legend = "bottom")
```
...
ggsave("results.jpeg", path = "Slides/Rsweave")
```
