...
```
After box-cox transformation & stepwise AIC selection, the final model we choose is "outcome_3 ~ dde_env_bc + pcb_env_bc + race_agg + score_occupation + center + score_income + score_education + smoking_status + dde_env_bc:race_agg+pcb_env_bc:race_agg".
```{r}
...
m <- stan_polr(
  outcome_3 ~ 
  dde_env_bc + pcb_env_bc + race_agg + score_occupation + center + score_income + score_education +smoking_status+dde_env_bc:race_agg+pcb_env_bc:race_agg,
  d,
  iter=8*1e3,
  chains=1,
  method = "logistic", 
