...
In order to incorporate the model uncertainty into the analysis, we wish to adopt a Bayesian model averaging (BMA) approach. Yet, since we are not aware of existing computing resources to realize BMA on an ordinal logistic regression model, we instead conduct an forward and backward AIC-based variable selection procedure and use the selected variables as predictors in a Bayesian ordinal logistic regression model. The procedures selects $\text{DDE}_{\text{exposure}}$, $\text{PCB}_{\text{exposure}}$, smoke, center, race and the interactions between $(\text{DDE}_{\text{exposure}}$ and race and between $\text{PCB}_{\text{exposure}}$ and race, and drops the three scores, mother age, and the interactions between $\text{DDE}_{\text{exposure}}$ and center, and between $\text{PCB}_{\text{exposure}}$ and center. The Bayesian model was fit on stan using $10$ chains of $10,000$ iterations.
...
\figureref{fig:corr} presents the correlation matrix of the $12$ PCB measurements. We can observe that they are highly correlated with each other. This is not surprising since they correspond to breakdown products of PCB. This provides a rationale for aggregating these measurements into one variable that attempts to approximate the total amount of PCB in the women's blood (see \sectionref{sec:feature}). \figureref{fig:center} presents the distribution of gestational outcome across the different centers. There exists a wide spread of outcome among the centers: for instance, while center $31$ did not have any delivery that was \textit{dangerous}, more than a third of the deliveries realized in the centers $15, 62, 37$ occurred pre-term. \figureref{fig:pcb} shows the distribution of the estimated exposure to PCB and DDE per gestational outcome and per race. We can observe a negative weak association between gestational outcome and DDE. We also note that the distribution of the chemicals vary across the two race levels. This indicates that we need to control for race in the regression model in order to prevent the variable from acting like a confounder.
...
\newpage % ensures that all figures remain together in appendix A.
...
Since the ordinal data is used, the common residual plot model checking is not applicable here. Instead, the surrogate residual method suggested by (%$\ref{zz2017}$
