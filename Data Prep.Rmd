...
ggsave("pcb_dde_per_gest.jpeg", path = "Slides/Rsweave")
```
...
ggsave("pcb_corr.jpeg", path = "Slides/Rsweave")
```
...
ggsave("outcome_per_center.jpeg", path = "Slides/Rsweave")
```
...
```
After box-cox transformation & stepwise AIC selection, the final model we choose is "outcome_3 ~ dde_env_bc + pcb_env_bc + race_agg + center + smoking_status + dde_env_bc:race_agg + pcb_env_bc:race_agg".
```{r}
...
d_boxcox_check= d %>% 
  dplyr::select(outcome_3,race,maternal_age,score_occupation,
                center,score_income,score_education, smoking_status)
# Compute the boxcox
...
for(lambda in lambda_list){
  ...
  d_boxcox_check$pcb_check = d$pcb/boxcox_transfomation(d$fat, lambda)
    model_lambda = polr(outcome_3 ~ . + center*dde_check 
                                   + center*pcb_check +
                                     race*dde_check +
...
```{r}
m <- stan_polr(
  outcome_3 ~ dde_env_bc + pcb_env_bc + race_agg + center + smoking_status + dde_env_bc:race_agg + pcb_env_bc:race_agg,
  d,
  iter=8*1e3,
  chains=1,
  method = "logistic", 
...
```{r}
tibble(dde = seq(0,k-1),
       p0 = prob0, p1 = prob1) %>%
  gather(Outcome, Probability,p0,p1) %>%
  mutate(low = c(prob0_quants[1,], prob1_quants[1,]),
         upp = c(prob0_quants[2,], prob1_quants[2,])) %>%
  ggplot(aes(x=dde)) +
  geom_line(aes(y=Probability, col = Outcome)) +
  geom_ribbon(aes(ymin = low, ymax = upp, fill = Outcome), alpha = 0.1) +
  theme(legend.position = "bottom")+
  facet_grid(~"something")
```
...
prob1_quants=apply(prob1_draws,1,quantile,prob=c(.05,.95)) ##Calculate the quantiles of P(Y=1)
#####Plot
tibble(pcb = seq(-0.3,.8,length.out =k ),
       p0 = prob0, p1 = prob1) %>%
  gather(Outcome, Probability,p0,p1) %>%
  mutate(low = c(prob0_quants[1,], prob1_quants[1,]),
         upp = c(prob0_quants[2,], prob1_quants[2,])) %>%
